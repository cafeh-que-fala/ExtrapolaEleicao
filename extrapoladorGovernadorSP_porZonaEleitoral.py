...
def wrapper(role,uf,city_code,election_number,previous_election_number,url_generator,valid_voting_number_tuple,votes,ballots_status):
...
t0 = time.time()
c = 0 
len_municipios = len(municipios_SP)
for city_code in municipios_SP:
    c+=1
   
    remaining = len_municipios - c + 1
    eta = (time.time() - t0)/c *remaining
    print(f"\r{str(c).zfill(5)} / {len_municipios} -- ETA: {eta:.1f}s   ",end="")
    wrapper("3","sp",city_code,"547", "546", url_generator,("10","13"),votes,ballots_status)

t1 = time.time()

votes["extrapolated_pct"]["10"] = votes["extrapolated"]["10"] / (votes["extrapolated"]["13"] + votes["extrapolated"]["10"])
votes["extrapolated_pct"]["13"] = votes["extrapolated"]["13"] / (votes["extrapolated"]["13"] + votes["extrapolated"]["10"])
print()
print(votes)
